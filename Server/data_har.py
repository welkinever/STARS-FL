...
def fetch_dataset(data_name, subset):
    ...
    if data_name == 'MNIST':
        ...
        dataset['train']=HARDataset('./data/har',train=True,transform=None)
        dataset['test']=HARDataset('./data/har',train=False,transform=None)
    else:
...
def non_iid(dataset, num_users, label_split=None):
    if cfg['data_name'] in ['har']:
        with open("./data/har/har_dataset_20.pkl","rb") as f:
            data_split=dill.load(f)
        label_split=[[0,1,2,3,4,5] for i in range(0,20)]
        return data_split,label_split
