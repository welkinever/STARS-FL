...
def transformer(model_rate=1):
    ...
    #print(f"总参数数量: {num_parameters}")
    #memory_estimate = estimate_model_memory(model)
    #print(f"预计内存占用: {memory_estimate / (1024**2):.2f} MB")
    #
...
def estimate_model_memory(model):
    parameters_size = sum(p.numel() * p.element_size() for p in model.parameters())
    buffer_size = sum(b.numel() * b.element_size() for b in model.buffers())
    total_size = parameters_size + buffer_size
    return total_size